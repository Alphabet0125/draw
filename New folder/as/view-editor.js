...
document.addEventListener('DOMContentLoaded', function() {
    initializeCanvas(currentFileId);
});
...
function initializeCanvas(fileId) {
    ...
    img.onload = function() {
        ...
        // ตั้งค่าขนาด canvas เป็นขนาดจริง
        canvas.width = originalWidth;
        canvas.height = originalHeight;
        
        // วาดรูปภาพต้นฉบับด้วยขนาดจริง 1:1
    ...
    img.onerror = function() {
        document.getElementById('canvasInfo').innerHTML = '<i class="fas fa-exclamation-triangle"></i> ไม่สามารถโหลดรูปภาพได้';
...
function updateCanvasInfo(width, height) {
    ...
    if (infoElement) {
        infoElement.innerHTML = `
            ...
            ขนาดรูปภาพ: ${width} × ${height} px (ขนาดจริง 1:1)
        `;
    }
}
...
function clearCanvas() {
    if (confirm('คุณต้องการล้างภาพวาดทั้งหมดหรือไม่?')) {
        const img = new Image();
        img.src = `preview.php?id=${currentFileId}&t=${new Date().getTime()}`;
        img.onload = function() {
            ctx.clearRect(0, 0, canvas.width, canvas.height);
...
function saveDrawing() {
    ...
    .then(data => {
        if (data.success) {
            ...
            // ถามว่าต้องการกลับไปหน้าไฟล์หรือไม่
            if (confirm('บันทึกสำเร็จ! ต้องการกลับไปหน้าไฟล์หรือไม่?')) {
...
// ป้องกันการ scroll ขณะใช้ touch บน canvas
